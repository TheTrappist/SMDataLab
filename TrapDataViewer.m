function varargout = TrapDataViewer(varargin)
% m-file file for TrapDataViewer.fig
%
%   TrapDataViewer is a program that displays all types of data files
%   generated by the Yildiz Lab optical traps
%
%   Adapted by Vladislav Belyy from Thomas Bilyard's original code
%   Last updated on 02/12/2013
%
% See also: GUIDE, GUIDATA, GUIHANDLES


% Last Modified by GUIDE v2.5 18-Jul-2013 15:24:45

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @TrapDataViewer_OpeningFcn, ...
    'gui_OutputFcn',  @TrapDataViewer_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before TrapDataViewer is made visible.
function TrapDataViewer_OpeningFcn(hObject, eventdata, handles, varargin) %#ok
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TrapDataViewer (see VARARGIN)
...
handles.display = struct('filtered', 0, 'trapPosLong', 0, ...
    'trapPosShort', 0, 'gridLines', 0,...
    'center', 0 ... % 0 = No centering, 1 = median, 2 = user values
...
handles.PSD2Data_Long = 0; % Rotated bead position along track
handles.PSD2Data_Short = 0; % Rotatedposition across track
handles.PSD2Data_Long_Filt = 0; % Filtered bead position
handles.PSD2Data_Short_Filt = 0;

handles.t_Filt = 0; % Filtered/decimated time vector

% Trap position; formerly handles.Flong and handles.Fshort
handles.trapPosLong=0;
handles.trapPosShort=0;
handles.piezoTrapPosLong=0;
handles.piezoTrapPosShort=0;
handles.trapTimeVector = 0;

handles.trapPosLong_Filt=0; % Filtered/decimated trap position
handles.trapPosShort_Filt=0;
handles.trapTimeVector_Filt = 0;
                                    
handles.currentPlotT = 0; % Current plot's time axis
handles.currentPlotTrapT = 0; %Current plot's trap time axis
...
handles.currentstyleTrapLong='m-';
handles.currentstyleTrapShort='g-';

handles.currentPath='C:\';

handles.ChLogic=[0 0 0 0];
handles.FeedbackConvPara=812.2;
...
% Choose default command line output for TrapDataViewer
...
if length(handles.rawPSD1Data_X) > 1 % Only plot data if some data file
    ...
    handles = BuildCurrentPSD1Data(hObject, handles);
...
if newIndex > 0
    ...
    if strcmp(extension, '.bin')
        handles = LoadNewBinDataFile(hObject, handles, path, newFilename);
    elseif strcmp(extension, '.ytd')
        handles = LoadNewYTDdataFile(hObject, handles, path, newFilename);
    end
...
if strcmp(currSelection, 'FIT')
    ...
    StepStatistics = SICstepFinder(x, minStepSize, noiseParam);
    %toc
    
    % Build step vector
    handles.stepVector = StepStatistics.StepFit;
    handles.stepVectorData = x; % Data (filtered/decimated) used for fit
    handles.stepVectorT = handles.currentPlotT(startTime:endTime);
...
function varargout = TrapDataViewer_OutputFcn(hObject, eventdata, handles)   %#ok<STOUT>
