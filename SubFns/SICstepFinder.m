function StepStatistics=SICstepFinder(x, minStepSize, noiseParam)
%% 
%----Info------------------------------------------------------------------
...
% x - input data
% noiseParam - skews the relative cost of adding a new step. Stepfinder is
% pure SIC if noiseParam is set to 1.
%
...
%% Main stepfitter loop
 while(SIC_new <= SIC_old) 
    ...
    % Determine proposed step size (added by VB on 11/8/2013)
    proposedStepSize= mean(x(StepInd+1:NextInd)) - ...
                                mean(x(PrevInd+1:StepInd));
    
    % only accept steps longer than MinPointsInStep and larger than minimum
    % step size
    if ( ((StepInd-PrevInd)>=MinPointsInStep && ...
            (NextInd-StepInd)>=MinPointsInStep) && ...
            abs(real(proposedStepSize)) > minStepSize)
    ...
    elseif abs(real(proposedStepSize)) < minStepSize  
        Metric(StepInd)=0;
        disp('a small step found and ignored')
